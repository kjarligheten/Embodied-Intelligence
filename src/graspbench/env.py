...
# MuJoCo environment for tabletop grasping with Franka Panda arm
...
class StepInfo:
    """Contact information from a simulation step.
...
class GraspEnv:
    """MuJoCo environment for tabletop grasping with RGB-D perception.

    This environment:
    - Simulates Franka Panda 7-DOF arm with parallel gripper
    - Manages tabletop objects and containers
    - Provides calibrated RGB-D camera observations
    - Handles physics simulation and collision detection
    - Supports random object placement within workspace

    Workspace: X [0.4, 0.6]m, Y [-0.25, 0.25]m, Z at table height
    """

    # Camera configuration
    CAMERA_NAMES = ("overhead", "front")  # RGB-D camera names
    CAMERA_SIZE = (256, 256)  # Render resolution [height, width]
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            control_substeps: Physics substeps per control command (higher = more accurate)
        """
    ...
    def control_dt(self) -> float:
        """Time step for control commands (physics timestep * substeps)."""
    ...
    def reset(self, task: TaskSpec) -> Observation:
        ...
        Args:
            task: Task specification with instruction and object configuration
        ...
        # Forward kinematics to update body positions from qpos
        mujoco.mj_forward(self.model, self.data)

        self.data.qvel[:] = 0.0
        mujoco.mj_forward(self.model, self.data)
        
        # Physics settling to let objects settle on the table
        for _ in range(120):
    ...
    def _sample_placements(
        ...
        # Sample within robot workspace: X [0.40, 0.60], Y [-0.24, 0.24]
        ...
        # Simple collision check - ensure objects are not too close
        min_distance = 0.08
        for _ in range(100):
            collision = False
    ...
    def observe(self) -> Observation:
        ...
        Returns:
            Observation containing RGB-D images, joint states, and instruction
    ...
    def step(self, command: JointPositionCommand | CartesianDeltaCommand) -> tuple[Observation, StepInfo]:
        ...
        Args:
            command: Joint position or Cartesian delta command to execute
            
        Returns:
            Tuple of (new observation, step info with contact data)
            
        Raises:
            TypeError: If Cartesian delta command is used without adapter
        """
    ...
    def is_success(self, target: str, lift_margin: float = 0.10) -> bool:
        """Check if target object has been successfully lifted.
        
        Args:
            target: Object identifier to check
            lift_margin: Required height above table for success
            
        Returns:
            True if object is above the lift margin
        """
    ...
    def is_object_placed(self, target: str, destination: str) -> bool:
        ...
        Returns:
            True if object is inside container and stable
        """
