...
# Visual Language Model for instruction understanding and target grounding
...
class VLMConfig:
    ...
    base_url: str  # Base URL of the VLM API (e.g., "https://api.example.com/v1")
    api_key: str  # API key for authentication
    model: str  # Model name to use
    timeout_s: float = 45.0  # Request timeout in seconds
...
class OpenAICompatibleVLM:
    ...
    def ground_target(
        ...
        prompt = (
            ...
            "Select exactly one detected candidate that best satisfies the instruction. "
            "Return a JSON object with exactly two fields: target_id and reason. "
            "CRITICAL RULES: "
            "1. The target_id field MUST be copied exactly from the detected_candidates list below. "
            "2. NEVER invent new object names or use descriptive names like 'red_square', 'blue_cylinder', etc. "
            "3. ONLY use the exact target_id values that appear in the detected_candidates list. "
            "4. Analyze the instruction for keywords like color (red, green, blue, yellow, orange, black, metal), "
            "   shape (cube, cylinder, box, round, brick), and object type (banana, apple, orange, bottle, can, scissors, marker). "
            "5. Match these keywords to the sam_prompt of each candidate to find the best match. "
            "6. If multiple candidates partially match, choose the one with the highest sam_score. "
            "7. If no candidate matches the instruction, choose the most similar one from the list. "
            "8. The detected_candidates list contains the ONLY valid target_id values you can use. "
            "9. Always copy-paste the target_id value - do not create or modify it. "
            "10. Your response will be validated - invalid target_ids will be rejected.\n"
