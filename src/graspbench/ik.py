...
class IKResult:
    """Result of inverse kinematics solving."""
    joint_position: np.ndarray  # Final joint positions [7 DOF]
    converged: bool  # Whether IK converged to solution
    position_error: float  # Final position error in meters
    orientation_error: float  # Final orientation error in radians
...
class DampedLeastSquaresIK:
    ...
    This solver uses damped least squares method to find joint angles
    that achieve a desired end-effector pose (position + orientation).
    """
    ...
    def __init__(
        ...
        Args:
            model: MuJoCo model containing the arm
            site_name: Name of the end-effector site to control
            damping: Damping factor for least squares (regularization)
    ...
    def solve(
        ...
        *,
        ...
        Args:
            initial_qpos: Initial joint positions [7] or full model qpos
            target_position: Desired end-effector position [x, y, z]
            target_quaternion: Desired end-effector orientation [w, x, y, z]
            max_iterations: Maximum number of IK iterations
            position_tolerance: Position error tolerance for convergence
            orientation_tolerance: Orientation error tolerance for convergence
            rest_qpos: Optional rest pose for null-space projection
            
        Returns:
            IKResult with joint positions and convergence status
        """
